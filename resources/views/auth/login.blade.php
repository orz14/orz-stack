        ...
        @if (env('GITHUB_CLIENT_ID') != null || env('GOOGLE_CLIENT_ID') != null)
            ...
            <div class="space-y-2">
                @if (env('GITHUB_CLIENT_ID') != null)
                    <x-button-link to="{{ route('login.provider', 'github') }}" class="w-full bg-black hover:bg-slate-800 focus:bg-slate-800 active:bg-black">
                ...
                @if (env('GOOGLE_CLIENT_ID') != null)
                    <x-button-link to="{{ route('login.provider', 'google') }}" class="w-full bg-red-700 hover:bg-red-600 focus:bg-red-600 active:bg-red-700">
                        <span class="flex items-center gap-x-1">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5 fill-current">
